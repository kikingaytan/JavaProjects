...
<!--  <script language="javascript" type="text/javascript" src="../plugins/jqplot.categoryAxisRenderer.js"></script>-->
  ...
  <script type="text/javascript" src="${ contextPath }/plugins/jqplot.enhancedLegendRenderer.js"></script>
...
function cargaDatos(){
	$.ajax({
		  url: 'cargaDatosPay.go',
		  success: function(data) {
		    $('.result').html(data);
			  $('#hiddenDatosPay').val(data);
		  }
	});
}		
...
<script id="example_4" type="text/javascript">$(document).ready(function(){
	var a = [[['A',10],['B',10],['C',1],['D', 4],['E', 50],['F',20]]];
	cargaDatos();
	//alert("CD=" + $('#hiddenDatosPay').val());
	
	//var a = $('#hiddenDatosPay').val();

    //TODO: hacer llamada ajax 
	
    plot4 = $.jqplot('chart4', a, {
      seriesDefaults:{
...
<script type="text/javascript">
  ...
  <body>
    <input type='hidden' id='hiddenDatosPay' value='' />
